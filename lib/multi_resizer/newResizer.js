...
export class RbfResizer extends BaseResizer {
  ...
  constructor(argmap) {
    super();
    if ('keyPoints' in argmap) {
      this.constructInEditor(argmap.image, argmap.keyPoints, argmap.horizontalSeamMap, argmap.verticalSeamMap);
    } else {
      this.constructInScript(argmap.image, argmap.metainfo);
    }
  }
   ...
   */
  constructInEditor(image, keyPoints, horizontalSeamMap, verticalSeamMap) {
    this.rbf = new RBF();
    this.image = image;
   ...
   */
  constructInScript(image, metainfo) {
    ...
    for (const keyPoint of metainfo.keys) {
      this.A.push(new NDArray([keyPoint.width, keyPoint.height]));
